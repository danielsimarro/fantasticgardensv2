...
- **Otra limitación de entorno conocida**: en la pestaña que controla el navegador de esta sesión, `document.visibilityState` es `"hidden"` (no es la pestaña visualmente en primer plano de Chrome, aunque `document.hasFocus()` diga que sí), así que el navegador pausa `requestAnimationFrame` ahí — cualquier interacción basada en rAF (magnetismo de CTAs, tilt 3D al ratón, etc.) no se anima visualmente al probarla en esta sesión aunque el código sea correcto. Para verificar la lógica sin depender de eso: parchear `window.requestAnimationFrame` a un `setTimeout` corto antes de re-disparar el módulo. No es un bug — en el navegador real del usuario la pestaña sí está en primer plano y rAF corre con normalidad.
...
- [x] Nuevas páginas de servicio "Soluciones integrales" y "Desbroce y limpieza de parcelas" creadas y enlazadas en el menú — contenido genérico de partida (sin fotos ni copy definitivo del cliente, salvo el comparador antes/después real ya añadido en "01 Qué incluye" de Desbroce), revisar antes de darlas por definitivas. El nombre "parcelas" (en vez de "terreno") se eligió por ser el término más buscado en el sector (ver competidores en Málaga/Costa del Sol)
